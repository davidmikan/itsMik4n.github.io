...
class Circle {
    ...
    update() {
        ...
        if (this.isGrounded() && this.v.y < 2) {
            // if (Math.abs(this.dy) < 0.5) {
            this.v.y = 0
            // }
            // ground friction
            this.v.x *= 1-this.gf
        } else if (this.isGrounded(this.v.y)) {
            // bounce
            this.v.y = Math.abs(this.v.y) * -1 * (1-this.f)
        } else {
            // gravity
            this.v.y += this.g
        }
        // left and right wall
        if ((this.x + this.v.x) > (canvas.width-this.r) || (this.x + this.v.x - this.r) < 0) {
            this.v.x *= -1 * this.cf
        }
        // top and bottom wall
        // if ((this.y + this.v.y) > (canvas.height-this.r) || (this.y + this.v.y - this.r) < 0) {
        //     this.v.y *= -1 * this.cf
        // }
        // air friction
